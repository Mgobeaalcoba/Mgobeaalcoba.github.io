...
4. Sube el `.md` al repositorio en `apps/web/content/posts/` vía **GitHub OAuth2** → dispara el CI/CD automáticamente
...
- El archivo `.md` se crea en `apps/web/content/posts/YYYY-MM-DD-{slug}.md` con frontmatter mínimo (`slug` + `date`)
- El push a GitHub dispara automáticamente el GitHub Actions `deploy.yml` (path `apps/web/content/posts/**`)
