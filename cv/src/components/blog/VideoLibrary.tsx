  ...
  return (
    <>
      ...
      <section className="signal-video-library">
        <div className="signal-video-library__tools"><label><Search size={17} /><input value={query} onChange={(event) => setQuery(event.target.value)} onBlur={() => { if (query.trim()) events.contentSearch('video', query.trim().length, filtered.length); }} placeholder={lang === 'es' ? 'Buscar videos…' : 'Search videos…'} /></label><div>{categories.map((value) => <button key={value} className={category === value ? 'is-active' : ''} onClick={() => { events.contentFilter('video', 'category', value); setCategory(value); }}>{value === 'all' ? (lang === 'es' ? 'Todos' : 'All') : value}</button>)}</div></div>
