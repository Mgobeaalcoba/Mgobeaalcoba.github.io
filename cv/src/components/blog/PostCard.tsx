  ...
  return (
    ...
    >
      <Link
        href={`/blog/${post.slug}/`}
        onClick={() =>
          events.blogPostCardClick(post.slug, post.title[lang], post.category)
        }
      ...
      >
        <article className="glass rounded-2xl p-6 glow-border hover:scale-[1.01] transition-all duration-200 h-full flex flex-col gap-4">
          ...
          <h3 className="font-bold text-gray-100 leading-snug group-hover:text-sky-400 transition-colors text-lg line-clamp-2 min-h-[3.5rem] flex items-center">
            {post.title[lang]}
          </h3>

          <p className="text-gray-400 text-sm leading-relaxed flex-1 line-clamp-4 min-h-[5rem]">
            {post.excerpt[lang]}
          </p>
